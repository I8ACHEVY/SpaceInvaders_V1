...
namespace SDL_Framework {
	...
	PhysicsManager::PhysicsManager() {
		...
		for (int i = 0; i < static_cast<unsigned int>
			(CollisionLayers::MaxLayers); i++) {

			mLayerMasks[i] = std::bitset<
	...
	void PhysicsManager::UnregisterEntity(unsigned long id) {
		...
		for (int i = 0; i < static_cast<unsigned int>(CollisionLayers::MaxLayers) 
			&& !found; i++) {
