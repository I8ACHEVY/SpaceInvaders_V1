...
namespace SDL_Framework {
	...
	inline bool ColliderVsColliderCheck(Collider* collider1, Collider* collider2) {
		if (collider1->GetType() == Collider::ColliderType::Circle &&
			...
			return CircleVsCircleCollision(static_cast<CircleCollider*>(collider1),
				static_cast<CircleCollider*>(collider2));
		}
		else if (collider1->GetType() == Collider::ColliderType::Box &&
			collider2->GetType() == Collider::ColliderType::Circle) {

			return BoxVsCircleCollision(static_cast<BoxCollider*>(collider1),
				static_cast<CircleCollider*>(collider2));
		}
		else if (collider1->GetType() == Collider::ColliderType::Circle &&
			collider2->GetType() == Collider::ColliderType::Box) {

			return BoxVsCircleCollision(static_cast<BoxCollider*>(collider2),
				static_cast<CircleCollider*>(collider1));
		}
		else if (collider1->GetType() == Collider::ColliderType::Box &&
			collider2->GetType() == Collider::ColliderType::Box) {

			return BoxVsBoxCollision(static_cast<BoxCollider*>(collider1),
				static_cast<BoxCollider*>(collider2));
		}
		else {
			return false;
		}
